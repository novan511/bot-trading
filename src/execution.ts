 ...
 */
function tradeFingerprint(t: TradeRecord): string {
 ...
 */
function tradeContentKey(t: TradeRecord): string {
  const num = (v: number, fallback: string) => Number.isFinite(v) ? v.toPrecision(10) : fallback;
  return [
    t.symbol,
    t.side,
...
export class ExecutionEngine {
  ...
  private lastStopLossTimes: Map<string, number> = new Map();

  // Auto risk guards: rolling-expectancy kill switch & consecutive-loss breaker per symbol
  private symbolSuspensions: Map<string, { until: number; cause: string }> = new Map();
  ...
  public evaluatePositions(symbol: string, currentBid: number, currentAsk: number) {
    ...
    for (const position of [...positions]) {
      ...
      if (position.highestPrice === undefined || currentBid > position.highestPrice) position.highestPrice = currentBid;
      if (position.lowestPrice === undefined || currentAsk < position.lowestPrice) position.lowestPrice = currentAsk;
  ...
  public async executeSignal(signal: TradeSignal) {
    ...
    const baseQuantity = this.riskManager.calculatePositionSize(
      this.stats,
      signal.symbol,
      atr,
      entryPrice,
   ...
   */
  private getTotalGrossNotionalUsd(): number {
    let total = 0;
   ...
   */
  private evaluateSymbolRiskGuards(symbol: string, result: 'WIN' | 'LOSS' | 'BREAKEVEN') {
   ...
   */
  private checkSymbolSuspension(symbol: string): { allowed: boolean; reason?: string } {
    const susp = this.symbolSuspensions.get(symbol);
    if (!susp) return { allowed: true };
   ...
   */
  public getSymbolGuardStatus(): Record<string, { suspendedUntil: number; cause: string }> {
    const out: Record<string, { suspendedUntil: number; cause: string }> = {};
  ...
  private checkPartialTPs(position: Position, currentBid: number, currentAsk: number) {
    if (!position.partialTPs || !position.remainingQty) return;
    if (position.partialTPs.every(tp => tp.isTriggered)) return;
   ...
   */
  public importTrades(records: any[]): { imported: number; skipped: number; skippedDuplicate: number; skippedInvalid: number } {
    ...
    for (const raw of records || []) {
      ...
      if (!rec.symbol || !isFinite(rec.entryPrice) || !isFinite(rec.exitPrice) ||
          !isFinite(rec.quantity) || !isFinite(rec.netProfitUsd) || !isFinite(rec.exitTime)) { skippedInvalid++; continue; }
    ...
    if (toInsert.length > 0) {
      ...
      for (const r of toInsert) {
        try {
          if (this.database) this.database.saveTrade(r);
        } catch {}
      }
  ...
  private recalculateStats() {
    ...
    for (const t of this.tradesHistory) {
      ...
      if (t.result === 'WIN') { wins++; grossWins += Math.max(0, t.grossProfitUsd); }
      else if (t.result === 'LOSS') { losses++; grossLosses += Math.abs(Math.min(0, t.grossProfitUsd)); }
    }
