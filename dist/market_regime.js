...
function timeframeTrend(candles) {
    if (!candles || candles.length < CONFIG.REGIME_MIN_CANDLES_MACRO)
        return 'NEUTRAL';
    ...
    if (emaFast > emaSlow && lastClose >= emaFast && (emaFastPrev === null || emaFast >= emaFastPrev))
        return 'BULLISH';
    if (emaFast < emaSlow && lastClose <= emaFast && (emaFastPrev === null || emaFast <= emaFastPrev))
        return 'BEARISH';
    return 'NEUTRAL';
...
export class MarketRegimeDetector {
     ...
     */
    updateCandleContext(symbol, candles) {
        if (!candles.d1?.length && !candles.h4?.length && !candles.m15?.length)
            return;
        ...
        const macroTrend = dailyTrend === 'BULLISH' && h4Trend === 'BULLISH' ? 'BULLISH' :
