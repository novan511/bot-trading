...
function normalizeDisplayRow(row) {
    ...
    if (!isFinite(exitTime) || !isFinite(entryPrice) || !isFinite(netProfitUsd))
        return null;
    ...
    if (isFinite(mfeNum))
        normalized.mfePct = mfeNum / 100;
    const maeNum = parseImportNumber(row.maePctStr);
    if (isFinite(maeNum))
        normalized.maePct = maeNum / 100;
    return normalized;
}
export function parseImportPayload(format, raw) {
    let rows;
    if (format === 'json' || (!format && raw.trim().startsWith('['))) {
        const arr = JSON.parse(raw);
        if (!Array.isArray(arr))
            throw new Error('JSON harus berupa array of trades');
        rows = arr.map((r) => {
            // Full TradeRecord objects pass through; simplified display objects get normalized
            if (r && typeof r === 'object' && r.exitTime !== undefined && r.netProfitUsd !== undefined && r.entryPrice !== undefined && typeof r.entryPrice === 'number')
                return r;
    ...
    if (valid.length === 0)
        throw new Error('Tidak ada baris trade yang valid ditemukan di file');
    return valid;
}
// ============================================================
...
const CHANGELOG_CATEGORIES = ['CONFIG', 'STRATEGY', 'RISK', 'EXIT', 'INFRA', 'DATA', 'UI'];
...
function writeChangelogFile(entries) {
    const filePath = path.join(process.cwd(), CHANGELOG_FILE);
...
function computePnlBlock(trades, balanceUsd) {
    ...
    for (const t of sorted) {
        ...
        if (equity > peak)
            peak = equity;
        if (peak > 0)
            maxDDPct = Math.max(maxDDPct, ((peak - equity) / peak) * 100);
    }
    return {
...
function buildGoLiveReport(allTrades, guards, btcDailyCandles, balanceUsd) {
...
export class WebDashboardServer {
    ...
    constructor(port = 3000, exchange) {
        ...
        this.server = http.createServer((req, res) => {
            ...
            if (req.method === 'GET' && url.startsWith('/api/market-context')) {
                ...
                let payload = { symbols: Object.keys(ctx), timeframes: ['1M', '1w', '1d', '4h', '1h', '30m', '15m', '5m'] };
            ...
            if (req.method === 'GET' && url.startsWith('/api/chart-indicators')) {
                ...
                const payload = {
                    ...
                    fvgs: (ind?.fvgs || []).filter((f) => !f.isFilled).slice(-6),
                    poc: ind?.poc ?? null,
                ...
                req.on('end', () => {
                    try {
                        ...
                        if (!CHANGELOG_CATEGORIES.includes(category))
                            throw new Error(`Kategori harus salah satu dari: ${CHANGELOG_CATEGORIES.join(', ')}`);
                        const entry = {
    ...
    setMarketContextProvider(provider) {
    ...
    handleHistoricalDataApi(req, res) {
