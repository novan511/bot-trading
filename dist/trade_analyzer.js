export class TradeAnalyzer {
    ...
    analyzeOBIThreshold(trades, current) {
        const thresholds = [0.15, 0.18, 0.20, 0.22, 0.25, 0.30];
        let bestThreshold = current;
        let bestScore = -Infinity;
        const results = [];
        for (const threshold of thresholds) {
            const tradesAtThresh = trades.filter(t => t.obi !== undefined ? t.obi > threshold : true);
            if (tradesAtThresh.length < 3)
                continue;
        ...
        if (bestThreshold === current || results.length === 0)
            return null;
        ...
        const confidence = (bestResult?.trades || 0) > 10 ? 'HIGH' : bestResult && bestResult.trades > 5 ? 'MEDIUM' : 'LOW';
        return {
    ...
    analyzeZScoreThreshold(trades, current) {
        const thresholds = [0.6, 0.7, 0.8, 0.9, 1.0, 1.2];
        let bestThreshold = current;
        let bestScore = -Infinity;
        const results = [];
        for (const threshold of thresholds) {
            const tradesAtThresh = trades.filter(t => t.zScore !== undefined ? Math.abs(t.zScore) > threshold : true);
            if (tradesAtThresh.length < 3)
                continue;
            const wr = tradesAtThresh.filter(t => t.netProfitUsd > 0).length / tradesAtThresh.length;
            const avgP = tradesAtThresh.reduce((s, t) => s + t.netProfitUsd, 0) / tradesAtThresh.length;
    ...
    analyzeTpSlRatio(trades, currentTp, currentSl) {
        ...
        for (const ratio of ratios) {
            const simulated = trades.map(t => {
                const side = t.side;
                if (side === 'BUY') {
                    if (t.maxPrice && t.maxPrice >= ratio.tp)
                        return ratio.tp;
                    if (t.minPrice && t.minPrice <= ratio.sl)
                        return -ratio.sl;
                }
                else {
                    if (t.minPrice && t.minPrice <= ratio.tp)
                        return ratio.tp;
                    if (t.maxPrice && t.maxPrice >= ratio.sl)
                        return -ratio.sl;
                }
        ...
        if (bestRatio.tp === currentTp && bestRatio.sl === currentSl)
            return null;
    ...
    analyzeConfirmations(trades, current) {
        const confs = [0, 1, 2, 3, 4, 5];
        let bestConf = current;
        let bestScore = -Infinity;
        const results = [];
        for (const minConf of confs) {
            const tradesAtConf = trades.filter(t => t.confirmations !== undefined ? t.confirmations >= minConf : true);
            if (tradesAtConf.length < 3)
                continue;
        ...
        if (bestConf === current || results.length === 0)
            return null;
    ...
    analyzeSrThreshold(trades, current) {
        const thresholds = [0.005, 0.008, 0.01, 0.015, 0.02];
        let bestThreshold = current;
        let bestScore = -Infinity;
        const results = [];
        for (const threshold of thresholds) {
            const tradesAtThresh = trades.filter(t => t.srDistance !== undefined ? t.srDistance <= threshold : true);
            if (tradesAtThresh.length < 3)
                continue;
            const wr = tradesAtThresh.filter(t => t.netProfitUsd > 0).length / tradesAtThresh.length;
            const avgP = tradesAtThresh.reduce((s, t) => s + t.netProfitUsd, 0) / tradesAtThresh.length;
